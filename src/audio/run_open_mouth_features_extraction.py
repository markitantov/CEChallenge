...
def calculate_triangle_area(landmark1: NormalizedLandmark, landmark2: NormalizedLandmark, landmark3: NormalizedLandmark) -> float:
...
def extract_surface_area(path_to_images: str, path_to_landmarks: str, speaker_id: int = 0) -> None:
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for folder in tqdm(os.listdir(path_to_images)):
            pd_lips = []
            for idx, file in enumerate(sorted(glob.glob(os.path.join(path_to_images, folder, str(speaker_id).zfill(2), '*.jpg')))):
...
if    __name__ == '__main__':
    path_to_images = "/media/maxim/Databases/C-EXPR-DB/faces/"
    path_to_landmarks = "/media/maxim/Databases/C-EXPR-DB/features/open_mouth/"
