...
class CExprDataset(Dataset):
    ...
    Args:
        ...
        vad_file_path (str): Path to vad file TODO
    ...
    def find_video_info(self, lab_filename: str) -> tuple[float, float]:
    ...
    def prepare_data(self) -> None:
        ...
        for fp in os.listdir(self.video_root):
            ...
            v_fps, num_frames = self.find_video_info(fp)
            if self.labels_root:
                ...
                labs = pd.DataFrame(data=np.full((int(num_frames), 1), -2), columns=['label'])
                labs['lab_id'] = labs.index + 1
            

            features = pd.read_csv(os.path.join(self.features_root, fp.replace('mp4', 'csv')), 
                                   sep=',', 
...
if __name__ == "__main__":
    ...
    ced = CExprDataset(audio_root=audio_root, video_root=video_root, 
                       labels_root=None, features_root=features_root, 
