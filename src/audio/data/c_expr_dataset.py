...
class CExprDataset(Dataset):
    ...
    def parse_features(self, 
                       ...
                       lab_filename: str,
                       frame_rate: float) -> tuple[list[dict], list]:
        ...
        lab_feat_df = lab_feat_df[(lab_feat_df['expr'] != -1) & ((s.groupby(s).transform('size') < mouth_open_threshold) | (lab_feat_df['mouth_open'] == 1))]
        ...
        for idx, s in sequences.items():
            ...
            for seg in range(0, len(frames), shift):
                ...
                if len(expr_window) < min_w_len: # if less than min_w_len: get last -max_w_len elements
...
if __name__ == "__main__":
    ced = CExprDataset(audio_root=c_config['C_WAV_ROOT'], 
